...
namespace UnityEngine.UI
{
    ...
    {
        [NonSerialized]
        public const int AnimationPlayNum = 10;//播放平移动画的时候先挪动的数量
        [NonSerialized]
        public static bool AnimationPlayOver = false;//是否完成了平移动画
        //==========LoopScrollRect==========
        ...
        public void SetData<T>(IEnumerable<T> datas)
        {
            dataSource = new LoopScrollArraySource<T>(datas.ToArray());
        }

        public void SetData<T>(List<T> datas, Action<int, Transform, T> updateCallback)
        {
            dataSource = new LoopScrollCallbackSource<T>(datas, updateCallback);
        }
        ...
        public event Action onInitAnimationStart;
        public event Action onInitAnimationComplete;
        public Action<GameObject, TweenCallback> PlayItemInitAnimation;
        ...
        public float showItemTime = 0.2f;
        public float showItemDelay = 0.15f;
        public GameObject disableRaycastObj;
        ...
        public void Destroy()
        {
            ...
            {
                if (disableRaycastObj != null)
                {
                    disableRaycastObj.SetActive(false);
                }
            }
        }
        ...
        public void RefillCells(int offset = 0, bool needShow = true)
        {
            if (!Application.isPlaying || prefabSource == null)
                return;

            StopMovement();
            offset -= offset % contentConstraintCount;
            ...
            while (sizeToFill > sizeFilled)
            {
                float size = reverseDirection ? NewItemAtStart(needShow) : NewItemAtEnd(needShow);
                if (size <= 0) break;
                sizeFilled += size;
            }
        ...
        private void DoAnimation()
        {
            ...
            if (disableRaycastObj != null)
            {
                disableRaycastObj.SetActive(true);
            }

            int oneLineCount = 0;

            if (content.GetComponent<HorizontalLayoutGroup>() != null || content.GetComponent<VerticalLayoutGroup>() != null)
            {
                oneLineCount = content.childCount;
            }
            else if (content.GetComponent<GridLayoutGroup>() != null)
            {
                ...
                {
                    Debug.LogError(gameObject.name + "上的GridLayoutGroup组件的Constraint设置为Flexible");
                ...
                oneLineCount = gridLayoutGroup.constraintCount;
            }
            else
            {
                Debug.LogError(gameObject.name + "上未找到Layout组件");
            ...
            for (int i = 0; i < content.childCount; i++)
            {
                SetItemAnimation(content.GetChild(i).gameObject, i, oneLineCount, content.childCount, () =>
                {
                    if (disableRaycastObj != null)
                    {
                        disableRaycastObj.SetActive(false);
                    ...
                    onInitAnimationComplete?.Invoke();
                });
            }
        }

        private void SetItemAnimation(GameObject item, int index, int oneLineCount, int allCount, Action callback)
        {
            item.transform.localScale = Vector3.zero;
            item.SetActive(true);

            var a = index / oneLineCount;
            var b = index % oneLineCount;
            ...
            if (PlayItemInitAnimation != null)
            {
                PlayItemInitAnimation.Invoke(item, tweenCallback);
            }
            else
            {
                item.transform.DOScale(Vector3.one, showItemTime).SetDelay(b * showItemDelay + a * showItemDelay).OnComplete(tweenCallback);
            }
        }

        protected float NewItemAtStart(bool needShow = true)
        {
            if (totalCount >= 0 && itemTypeStart - contentConstraintCount < 0)
            {
                return 0;
            ...
            {
                itemTypeStart--;
                RectTransform newItem = InstantiateNextItem(itemTypeStart, needShow);
        ...
        protected float NewItemAtEnd(bool needShow = true)
        {
            ...
            {
                RectTransform newItem = InstantiateNextItem(itemTypeEnd, needShow);
                size = Mathf.Max(GetSize(newItem), size);
                itemTypeEnd++;
                if (totalCount >= 0 && itemTypeEnd >= totalCount)
                {
        ...
        private RectTransform InstantiateNextItem(int itemIdx, bool needShow = true)
        {
            RectTransform nextItem = prefabSource.GetObject().GetComponent<RectTransform>();
            nextItem.transform.SetParent(content, false);
            nextItem.gameObject.SetActive(needShow);
